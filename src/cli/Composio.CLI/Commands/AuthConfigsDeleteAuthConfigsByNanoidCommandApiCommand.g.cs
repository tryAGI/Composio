...
internal static partial class AuthConfigsDeleteAuthConfigsByNanoidCommandApiCommand
{
    ...
    private static Option<bool?> RevokeOnDelete { get; } = CliRuntime.CreateNullableBoolOption(
        ...
        description: @"When `true`, the delete also starts a background job that revokes the upstream credentials of every connected account in scope, and the response carries a `revoke_job_id`. Defaults to `false`. Revocation is irreversible — recovering a deleted entity does not restore working credentials.");
                    ...
                    private static string FormatResponse(ParseResult parseResult, global::Composio.DeleteAuthConfigsByNanoidResponse value, global::System.Text.Json.Serialization.JsonSerializerContext context, bool truncateLongStrings)
                    {
                        string? text = null;
                    ...
                    static partial void CustomizeResponseText(ParseResult parseResult, global::Composio.DeleteAuthConfigsByNanoidResponse value, ref string? text);
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.AuthConfigs.DeleteAuthConfigsByNanoidAsync(
                                    nanoid: nanoid,
