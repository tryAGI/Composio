...
internal static partial class ProjectsGetOrgOwnerProjectListCommandApiCommand
{
        ...
        name: @"--cursor")
    {
        Description = @"Cursor for pagination. The cursor is a base64 encoded string of the page and limit. The page is the page number and the limit is the number of items per page. The cursor is used to paginate through the items. The cursor is not required for the first page.",
                    ...
                    private static string FormatResponse(ParseResult parseResult, global::Composio.GetOrgOwnerProjectListResponse value, global::System.Text.Json.Serialization.JsonSerializerContext context, bool truncateLongStrings)
                    {
                        string? text = null;
                    ...
                    static partial void CustomizeResponseText(ParseResult parseResult, global::Composio.GetOrgOwnerProjectListResponse value, ref string? text);
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.Projects.GetOrgOwnerProjectListAsync(
                                    limit: limit,
                                    cursor: cursor,
